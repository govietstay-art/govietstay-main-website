...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{
 ...
 if(signature)return signatureMetadata('en',signature);
 return buildExperienceMetadata('en',slug);
}
export default async function Page({params}:{params:Promise<{slug:string}>}){
 const {slug}=await params;
 const signature=findSignature(slug);
 if(signature)return <DaNangSignaturePage locale="en" experience={signature}/>;
 const draft=getExperience(slug);
 if(!draft)notFound();
 return <LocalExperienceDraftPage locale="en" experience={draft}/>;
}
