...
export async function GET(request:NextRequest,context:{params:Promise<{merchantCode:string;offerCode:string;partnerCode:string}>}){
  const {merchantCode,offerCode,partnerCode}=await context.params;
  const merchant=clean(merchantCode),offer=clean(offerCode),partner=clean(partnerCode);
  if(!merchant||!offer||!partner)return NextResponse.redirect(new URL("/",FALLBACK),307);

  try{
    const resolved=await rpc("resolve_merchant_offer_bridge",{p_merchant_code:merchant,p_offer_code:offer,p_partner_code:partner});
    ...
    void rpc("record_merchant_offer_visit",{
      p_merchant_code:merchant,p_offer_code:offer,p_partner_code:partner,
      p_visitor_id:visitor,p_page_url:request.url,p_referrer:request.headers.get("referer"),
    ...
    if(target.hostname.endsWith("govietstay.com")){
      ...
      response.cookies.set(COOKIE,`${merchant}|O:${offer}|${partner}`,{
