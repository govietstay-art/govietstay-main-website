...
export default function YandexSeoPanel({ supabase, days }: any) {
  ...
  const overview = useMemo(() => {
    ...
    const weighted = daily.reduce(
      (s, r) => s + n(r.avg_show_position) * Math.max(1, n(r.impressions)),
      0
    );
  ...
  const russianQueries = useMemo(
    () => queries.filter((q: any) => isCyrillic(q.query_text)),
    [queries]
  );
  ...
  return <>
    <div className="gva-section-head">
      <div>
        <h2>Yandex Webmaster → Supabase</h2>
        <div className="gva-mini">
          {run?.completed_at
            ? <>Lần đồng bộ: <b>{new Date(run.completed_at).toLocaleString("vi-VN")}</b> · {run.status}</>
            : <>Chưa có snapshot đúng kỳ {range.startDate} → {range.endDate}.</>}
        </div>
      </div>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <button className="gva-btn secondary" onClick={() => callYandex("test")} disabled={testing || syncing}>
          {testing ? "Đang test…" : "Test Yandex Webmaster"}
        </button>
    ...
    <div className="gva-analytics-note">
      Theo dõi Yandex Search giống Google Search Console: impressions, clicks, CTR, vị trí và từ khóa.
      Visitors, nguồn traffic và WhatsApp đã có sẵn ở tab Analytics / Marketing nên không cần phụ thuộc Yandex Metrica.
      OAuth token chỉ nằm trong Vercel Secret.
    </div>
    ...
    <div className="gva-grid2 gva-seo-grid">
      <div className="gva-card">
        <div className="gva-section-head">
          ...
          <div className="gva-mini">{russianQueries.length} query</div>
        </div>
        <QueryTable rows={russianQueries} empty="Chưa có Russian query trong snapshot." />
      </div>

      <div className="gva-card">
        <div className="gva-section-head">
          <h3>Top Yandex Queries</h3>
          <div className="gva-mini">Top theo impressions</div>
        </div>
        <QueryTable rows={queries} empty="Bấm Đồng bộ Yandex để lấy queries." />
      </div>
    </div>
    ...
    <div className="gva-card" style={{ marginTop: 15 }}>
      ...
      <div className="gva-table-wrap">
        <table className="gva-table">
          <thead>
            <tr><th>Date</th><th>Impressions</th><th>Clicks</th><th>CTR</th><th>Position</th></tr>
          </thead>
          <tbody>
