...
const SUPPLIER_MEDIA_SOURCE = "John’s Tours partner media library";
  ...
  return <main className="min-h-screen bg-[#f7f1e5] text-[#082f2b]">
    <section className="relative overflow-hidden bg-[#052f2d] text-white">
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-16 md:px-8 lg:grid-cols-[1.02fr_.98fr] lg:items-center lg:py-24">
        ...
        <div className="relative min-h-[360px] overflow-hidden rounded-[2rem] border border-white/15 shadow-2xl md:min-h-[480px]"><SupplierImage src={JOHNS_ISLAND} fallback="/tour/phuquoc/tour-01-1.jpg" alt="John's Tours Phu Quoc speedboat" className="absolute inset-0 h-full w-full object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-[#052f2d]/85 via-transparent to-transparent"/><div className="absolute bottom-0 left-0 right-0 p-6 md:p-8"><p className="text-xs font-black uppercase tracking-[.16em] text-amber-300">Supplier media · {SUPPLIER_MEDIA_SOURCE}</p><p className="mt-2 text-2xl font-black">GoVietStay booking · John’s Tours operation</p></div></div>
      </div>
    </section>
    ...
    <section id="phuquoc-booking-form" className="scroll-mt-5 bg-[#062f2c] text-white"><div className="mx-auto max-w-7xl px-5 py-14 md:px-8 md:py-20"><div className="grid gap-8 lg:grid-cols-[1.08fr_.92fr]">
      ...
      <aside className="lg:pt-8"><div className="sticky top-4 rounded-[1.8rem] bg-white p-6 text-[#082f2b] shadow-2xl"><p className="text-xs font-black uppercase tracking-[.14em] text-emerald-700">{t.estimate}</p><div className="mt-2 text-3xl font-black">{selected ? money(estimate,language) : "—"}</div><p className="mt-2 text-xs leading-5 text-[#647a75]">{t.estimateNote}</p><div className="my-5 h-px bg-[#dfe9e4]"/><h3 className="text-xl font-black">{t.depositTitle}</h3><p className="mt-3 text-sm leading-6 text-[#496b67]">{t.depositText}</p><p className="mt-3 rounded-xl bg-[#edf7f2] p-3 text-sm font-black text-emerald-800">{t.methods}</p><div className="my-5 h-px bg-[#dfe9e4]"/><p className="text-sm leading-6 text-[#496b67]">{t.operator}</p></div></aside>
    </div></div></section>
