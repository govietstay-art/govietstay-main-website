...
export default function PhuQuocJohnsCatalog({language}:{language:Language}){
  ...
  return <main className="min-h-screen bg-[#f7f1e5] pb-24 text-[#082f2b] md:pb-0">
    <section className="relative overflow-hidden bg-[#052f2d] text-white">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-5 sm:py-14 md:px-8 lg:grid-cols-[1.02fr_.98fr] lg:items-center lg:gap-10 lg:py-20">
        <div><div className="mb-5 inline-flex max-w-[155px] overflow-hidden rounded-2xl bg-white p-2 shadow-lg sm:max-w-[175px]"><img src="/brand/govietstay-phu-quoc-logo.png" alt="GoVietStay Phu Quoc" className="h-auto w-full object-contain" /></div><p className="text-[11px] font-black uppercase tracking-[.2em] text-amber-300 sm:text-xs sm:tracking-[.22em]">{t.eyebrow}</p><h1 className="mt-4 max-w-4xl text-[2.45rem] font-black leading-[1.02] tracking-tight sm:mt-5 sm:text-5xl md:text-6xl lg:text-7xl">{t.title}</h1><p className="mt-5 max-w-3xl text-base leading-7 text-white/80 sm:mt-6 sm:text-lg sm:leading-8">{t.lead}</p><div className="mt-7 flex flex-col gap-3 sm:mt-8 sm:flex-row sm:flex-wrap"><a href="#joint-tours" className="w-full rounded-full bg-amber-300 px-6 py-3.5 text-center font-black text-[#07322f] sm:w-auto">{t.jointTitle}</a><a href="#phuquoc-booking-form" className="w-full rounded-full border border-white/35 px-6 py-3.5 text-center font-black text-white sm:w-auto">{t.book}</a></div></div>
        <div className="relative min-h-[300px] overflow-hidden rounded-[1.6rem] border border-white/15 shadow-2xl sm:min-h-[360px] sm:rounded-[2rem] md:min-h-[480px]"><SupplierImage src={JOHNS_ISLAND} fallback="/tour/phuquoc/tour-01-1.jpg" alt={language === "ru" ? "Острова Фукуока и бирюзовое море" : "Phu Quoc islands and turquoise sea"} className="absolute inset-0 h-full w-full object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-[#052f2d]/85 via-transparent to-transparent"/><div className="absolute bottom-0 left-0 right-0 p-6 md:p-8"><p className="text-[10px] font-black uppercase tracking-[.15em] text-amber-300 sm:text-xs">{language === "ru" ? "Фукуок · местная поддержка" : "Phu Quoc · local support"}</p><div className="mt-2 inline-flex max-w-[150px] overflow-hidden rounded-xl bg-white p-1.5 shadow-lg sm:max-w-[170px]"><img src="/brand/govietstay-phu-quoc-logo.png" alt="GoVietStay Phu Quoc" className="h-auto w-full object-contain" /></div></div></div>
      </div>
    </section>

    <section id="joint-tours" className="mx-auto max-w-7xl px-5 py-14 md:px-8 md:py-20"><div className="max-w-4xl"><p className="text-xs font-black uppercase tracking-[.2em] text-emerald-800">13 programs</p><h2 className="mt-3 text-3xl font-black tracking-tight sm:text-5xl">{t.jointTitle}</h2><p className="mt-4 text-lg leading-8 text-[#315b56]">{t.jointText}</p>
