...
$Host.UI.RawUI.WindowTitle = "GoVietStay Local Point V23"
...
try {
    ...
    Write-Host "   GOVIETSTAY LOCAL POINT V23 - WINDOWS INSTALLER" -ForegroundColor White
    Write-Host "===============================================================" -ForegroundColor DarkCyan
    Write-Host ""
    ...
    $backup = Join-Path $repoParent "BACKUP-LOCAL-POINT-V23-$stamp"
    ...
    Write-Step "3" "Giai nen truc tiep giao dien V23 vao website"
    ...
    if ($componentContent -notmatch "local-point-v23") {
        throw "File trong website khong phai ma nguon Local Point V23."
    }
    Write-Host "  OK  Da xac nhan local-point-v23 trong thu muc components." -ForegroundColor Green
    ...
    if ($hasPreStagedFiles) {
        ...
        Write-Step "6" "Commit rieng Local Point V23 va push len main"
        ...
        if ($diffExitCode -eq 1) {
            & git.exe commit -m "Update Local Point V23 experience"
            if ($LASTEXITCODE -ne 0) {
            ...
            Write-Host "  Ma V23 da co san; se push commit chua dong bo neu co." -ForegroundColor DarkGray
        }
        ...
        Write-Host "        DA BUILD, COMMIT VA PUSH LOCAL POINT V23" -ForegroundColor Green
        Write-Host "===============================================================" -ForegroundColor Green
        & git.exe log -1 --oneline
        Write-Host ""
        Write-Host "Vercel se tu dong deploy. Cuoi trang moi co chu LOCAL POINT V23." -ForegroundColor White
        $exitCode = 0
    }
}
catch {
